...
const SUMMARY_LINES = [
  "솔직히 말하면 지금 재회 가능성은 낮은 쪽에 가깝다. 상대가 완전히 마음을 닫은 건 아닌데, 그렇다고 다시 열린 것도 아니다. 지금 적극적으로 연락하면 열리기보다는 부담만 쌓일 가능성이 크다.",
  "재회 가능성은 중간 정도로 보인다. 끝낸 것처럼 보여도 잔상은 아직 남아 있다. 연락하면 안 된다는 게 아니라, 지금 무겁게 연락하면 안 된다는 뜻에 가깝다.",
  "가능성은 중간인데, 여지처럼 보이는 신호와 회피하는 신호가 섞여 있다. 장문이나 압박으로 들어가면 닫히고, 짧고 가벼운 톤이면 아주 얇은 틈이 남을 수도 있다.",
  "가능성은 중간에서 살짝 위인데, 상대 피드를 보면 다시 얽히기 싫어하는 기운이 더 크게 보인다. 무겁게 연락부터 밀면 비추고, 가벼운 한 줄 정도면 그나마 틈을 건드려볼 수 있다.",
] as const;

const BREAKUP_RESONANCE = [
  "지금 양쪽 사이에는 확인하고 싶은 마음과 거리를 두고 싶은 마음이 동시에 걸려 있다. 겉으로는 괜찮아 보여도 톤은 냉각된 상태라서, 접점을 늘리려 하면 방어부터 올라갈 수 있다.",
  "멀쩡해 보이는 건 감정이 사라진 게 아니라, 다시 흔들리는 상황 자체를 피하고 싶은 쪽에 가깝다. 특정 소재에서만 반응이 달라진다.",
  "한쪽은 접점을 늘리려 하고 한쪽은 속도를 줄이려 한다. 감정 문제라기보다는 서로 기대하는 속도가 맞지 않는 게 지금 핵심이다.",
  "급하게 끊어낸 건 아니고, 대화 자체에 지친 상태에 가깝다. 지금은 대화를 늘리는 것보다 부담을 안 주는 게 상대 기준에서 더 중요하다.",
] as const;

const SNAPSHOT_YOU = [
  "네 계정을 보면 감정이 없어진 게 아니라 새어나가는 부분만 좁혀둔 상태다. 스토리로 우회하다가 정작 DM은 못 보내는 패턴이 보인다.",
  "들키기 싫은 마음이 고백이나 장문으로 번지면, 상대한테는 대화가 아니라 압박으로 느껴질 수 있다.",
  "혼자 잘 살고 있다는 걸 자꾸 보여주려는 건 오히려 방어에 가깝다. 증명이 많을수록 아직 에너지가 거기에 남아 있다는 뜻이기도 하다.",
] as const;

const SNAPSHOT_THEM = [
  "상대는 열린 사람처럼 보여도, 다시 흔들리고 싶지 않아서 문을 살짝 잠가둔 상태로 읽힌다.",
  "상대 피드를 보면 연애 관련 캡션은 줄이고 경계를 세워둔 흔적이 보인다. 친절해 보인다고 열려 있다고 착각하면 오판이 된다.",
  "상대가 절대 안 돌아온다는 것도 아니고, 다시 열자는 것도 아닌 애매한 상태다. 새 사람 쪽으로 기우는 정도는 심층 분석에서 본다.",
] as const;

const FREE_WHY_AMBIGUOUS = [
  "지금 먼저 들어가면 불리한 이유는, 끝낸 것 같아도 잔상은 남아 있어서 한 가지 신호만 보고 될 것 같다고 판단하면 오판이 될 수 있기 때문이다. 이 점수는 된다는 뜻이 아니라, 지금 들어가면 상대 방어만 두꺼워질 수 있다는 걸 가리키고 있다.",
  "지금 먼저 들어가면 불리한 이유는, 여지처럼 보이는 신호와 방어적인 신호가 동시에 있기 때문이다. 성급하게 연락하면 상대한테는 확인이 아니라 부담으로 먼저 읽힐 수 있다.",
  "지금 먼저 들어가면 불리한 이유는, 완전히 끝난 것도 아니고 적극적으로 재시작하려는 것도 아닌 애매한 상태이기 때문이다. 이 애매함 자체가 지금은 속도 조절이 중요하다는 판정이다.",
] as const;

const FREE_KEY_VARIABLE = [
  "지금 관계를 바꾸는 변수는 연락 자체가 아니라, 그 연락이 부담으로 느껴지느냐 대화로 느껴지느냐의 차이다. 상대의 피로감, 자존심, 그리고 첫 문장 길이가 결과를 가른다.",
  "상대가 새로운 일상을 쌓아왔을수록, 첫 접점이 가볍지 않으면 문은 그냥 닫힌다. 지금 변수는 연락의 무게감이다.",
  "지금 변수는 네 감정의 높이와 상대 방어 사이의 간격이다. 그 간격을 좁히기 전에는 말을 해도 전달이 엇나갈 수밖에 없다.",
] as const;

const MY_TYPE_NAMES = [
  "정리한 척하면서 인스타는 계속 들여다보는 사람",
  "말로는 안 한다면서 스토리로 다 흘리는 사람",
  "먼저 보고 싶은데 상대 반응 먼저 확인하는 사람",
  "끊은 게 아니라 그냥 결론을 미뤄둔 사람",
] as const;

const THEIR_TYPE_NAMES = [
  "마음은 남았는데 다시 얽히는 건 질색인 사람",
  "열린 것처럼 보이지만 절대 먼저 연락 안 하는 사람",
  "관계보다 본인 루틴이 먼저인 사람",
  "완전히 닫지는 않았는데 피곤함이 앞서는 사람",
] as const;
