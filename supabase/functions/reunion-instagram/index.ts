...
{
  "waitUntil": object ({
    "recommendedDuration": string (짧은 한 줄. 예: "2-3주", "한 달 후", "지금 즉시 NO"),
    "timingScale": string ("immediate"/"early"/"moderate"/"late"/"too_late" 중 하나. 시각 게이지용.),
    "supportingReasons": string[] (3개. 그 시점이 적정인 이유. 상대 피드 데이터 인용.),
    "adjustmentSignals": array of 2-3 objects. 신호 변화 시 조정법. 각: {"condition": "상황 조건 한 줄", "action": "조정 행동 한 줄"}
  }),
  "toneReply": object ({
  ...
  "misunderstanding": object ({
    "coreMisconception": string (핵심 오해를 따옴표 형식 한 줄. 예: "답이 없다 = 미련 없다"),
    "signalMismatch": object ({"youSend": "내가 보내는 신호 한 줄", "theyReceive": "상대가 받는 신호 한 줄"}),
    "consequences": string[] (3개. 이 오해가 만드는 부정적 결과. 짧고 날카롭게.)
  }),
  ...
  "myDestroy": object ({
    "trapPatterns": array of 4 objects. 자주 빠지는 함정 패턴. 각: {"pattern": "짧은 행동 묘사 (5-10자)", "warning": "그러지 말아야 하는 이유 한 줄"},
    "coreInsight": string (모든 패턴을 관통하는 핵심 통찰 한 줄. 송수신 미스매치 형식 OK.)
  })
}
Rules:
...
${JSON.stringify(compatibility || {})}`;
  ...
  // waitUntil: structured object or fallback to string
  const wu = parsed.waitUntil;
  if (wu && typeof wu === "object" && typeof wu.recommendedDuration === "string") {
    result.waitUntil = JSON.stringify({
      recommendedDuration: wu.recommendedDuration.trim(),
      timingScale: typeof wu.timingScale === "string" ? wu.timingScale.trim() : "moderate",
      supportingReasons: Array.isArray(wu.supportingReasons) ? wu.supportingReasons.filter((x: any) => typeof x === "string").slice(0, 5) : [],
      adjustmentSignals: Array.isArray(wu.adjustmentSignals) ? wu.adjustmentSignals.filter((s: any) => s?.condition).map((s: any) => ({
        condition: typeof s.condition === "string" ? s.condition.trim() : "",
        action: typeof s.action === "string" ? s.action.trim() : "",
      })).slice(0, 4) : [],
    });
  } else {
    result.waitUntil = typeof wu === "string" ? wu.trim() : "";
  }

  // misunderstanding: structured object or fallback to string
  const mu = parsed.misunderstanding;
  if (mu && typeof mu === "object" && typeof mu.coreMisconception === "string") {
    result.misunderstanding = JSON.stringify({
      coreMisconception: mu.coreMisconception.trim(),
      signalMismatch: mu.signalMismatch && typeof mu.signalMismatch === "object" ? {
        youSend: typeof mu.signalMismatch.youSend === "string" ? mu.signalMismatch.youSend.trim() : "",
        theyReceive: typeof mu.signalMismatch.theyReceive === "string" ? mu.signalMismatch.theyReceive.trim() : "",
      } : { youSend: "", theyReceive: "" },
      consequences: Array.isArray(mu.consequences) ? mu.consequences.filter((x: any) => typeof x === "string").slice(0, 5) : [],
    });
  } else {
    result.misunderstanding = typeof mu === "string" ? mu.trim() : "";
  }

  // myDestroy: structured object or fallback to string
  const md = parsed.myDestroy;
  if (md && typeof md === "object" && Array.isArray(md.trapPatterns)) {
    result.myDestroy = JSON.stringify({
      trapPatterns: md.trapPatterns.filter((t: any) => t?.pattern).map((t: any) => ({
        pattern: typeof t.pattern === "string" ? t.pattern.trim() : "",
        warning: typeof t.warning === "string" ? t.warning.trim() : "",
      })).slice(0, 6),
      coreInsight: typeof md.coreInsight === "string" ? md.coreInsight.trim() : "",
    });
  } else {
    result.myDestroy = typeof md === "string" ? md.trim() : "";
  }
